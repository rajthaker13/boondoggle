...
function Workflows(props) {
  ...
  async function uploadTwitter(userData) {
    ...
    async function generateTwitterCRMData(dm) {
      const messagesString = dm.messages
        .map(
          (message) => `${message.name} (${message.username}): ${message.text}`
        )
        .join("\n");
      ...
      window.chrome.runtime.sendMessage(
        ...
        async function (response) {
          if (response && response.cookie != null) {
            ...
            console.log("MESAGE ARRAY", messageArray);
            const userProfile = messageArray[0].profile;
            console.log("user prof", userProfile);

            // const profileResponse = await axios.get(
            //   "https://vast-waters-56699-3595bd537b3a.herokuapp.com/https://nubela.co/proxycurl/api/v2/linkedin",
            //   {
            //     headers: {
            //       Authorization: `Bearer ${process.env.REACT_APP_ENRICHMENT_KEY}`,
            //     },
            //     params: {
            //       linkedin_profile_url: `https://linkedin.com/in/${userProfile}/`,
            //     },
            //   }
            // );
  ...
  async function uploadEmails() {
    ...
    await Promise.all(
      new_emails.map(async (email) => {
        if (email.customer) {
          const { title, summary, toDoTitle, toDoResponse } =
            await generateEmailCRMData(email, userEmail);
          const date = Date.now();
  ...
  function createNewEmail(new_emails, email, userEmail) {
    ...
    const obj = {
      id: email.id,
      customer:
        email.latestDraftOrMessage.from[0].email === userEmail
